...
function TaskItem({
  ...
  const message = taskProgressMessageText(task);
  return (
    <div
      className={cn(
        "rounded-md border p-2 space-y-1",
    ...
    >
      ...
      {isRunning ? <Progress value={task.progress} size="sm" tone="blue" /> : null}
      {detailed ? (
        <div className="grid grid-cols-2 gap-1.5 pt-1 text-[11px]">
          <InfoCell label="상태" value={message} />
          <InfoCell label="진행률" value={`${task.progress}%`} />
