...
def _existing_real_paths(paths: set[str]) -> set[str]:
...
def scan_and_register(settings: Settings, db: Session) -> dict[str, int]:
    ...
    # 이미 등록된 tile_path 집합 — O(1) 조회용. symlink 는 링크 경로를 DB에
    # 보존하되, 같은 원본을 가리키는 다른 링크의 중복 등록도 막는다.
    ...
    existing_real_paths = _existing_real_paths(existing_paths)
    ...
    for entry in sorted(root.rglob("*")):
        ...
        try:
            ...
            real_path = None
        if abs_path in existing_paths or (
            real_path is not None and real_path in existing_real_paths
        ) or _same_existing_file(entry, existing_paths):
            stats["skipped"] += 1
            continue
